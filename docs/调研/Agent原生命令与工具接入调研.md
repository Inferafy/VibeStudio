...
关联模块：[外部工具层：Agent 接入](../模块设计/外部工具层-Agent接入.md)、[本地能力层：会话运行](../模块设计/本地能力层-会话运行.md)。

当前状态：调研完成；目标版本、Windows 启动和真实能力仍需 M0 实测。
...
- VibeStudio 最终锁定的两个适配器版本和 Windows 启动入口。
- 新建、加载和恢复会话时，两类适配器各自上报的命令与配置列表。
- Claude Code、Codex 的工具、终端、文件、审批、提问、用量和取消支持范围。
